...
void loop() {
  ...
  client.loop();
  if (SendDEF) {
    //Envia valor do DEF para o MQTT
    snprintf(buffer, 5, "%d", DEF);
    client.publish(SUB_DEF, buffer);
    SendDEF = false;
  }
...
/***************************************************************
    CALLBACK PARA RECEBER VALOR DO DEF DO MQTT
***************************************************************/
...
boolean check(String str) {
  ...
  if (cmp.equals(str)) {
    if (DEF == ATUAL || DEF == (ATUAL + 1) || DEF == (ATUAL - 1)) {
      return true;
    } else {
      return false;
    }
